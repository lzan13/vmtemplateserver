...
exports.authAdmin = function (req, res, next) {
    ...
    if (!token) {
        return ep.emit('error', {code: config.code.err_token_invalid, message: 'auth_token_invalid'});
    }
    token = token.substring(7);
    jwt.verify(token, config.token.secret, function (error) {
        if (error) {
            if (error.name === 'TokenExpiredError') {
                return ep.emit('error', {code: config.code.err_token_expired, message: 'token_expired'});
            } else {
...
exports.authToken = function (req, res, next) {
    ...
    jwt.verify(token, config.token.secret, function (error) {
        if (error) {
            if (error.name === 'TokenExpiredError') {
                return ep.emit('error', {code: config.code.err_token_expired, message: 'auth_token_expired'});
            } else {
                return ep.emit('error', {code: config.code.err_token_invalid, message: 'auth_token_invalid'});
            }
        }
