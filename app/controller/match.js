...
class MatchController extends Controller {
  ...
  async destroyList() {
    ...
    // 调用 Service 进行业务处理
    await service.match.destroyList(params);
    // 设置响应内容和响应状态码
  ...
  async random() {
    const { ctx, service } = this;
    const params = ctx.params.permit('gender', 'type');
    const match = await service.match.random(params);
    // 设置响应内容和响应状态码
  ...
  async index() {
    const { ctx, service } = this;
    // 组装参数
    const params = ctx.params.permit('page', 'limit', 'gender', 'type');
    // 调用 Service 进行业务处理
