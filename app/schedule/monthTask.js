...
class MonthSubscription extends Subscription {
  ...
  static get schedule() {
    return {
      ...
      cron: '30 30 3 1 * *', // 指定时间点执行，这是设置每月1号凌晨03:30
...
async function resetUserAttr(ctx) {
  ...
  const result = await ctx.model.User.updateMany({ charmMonth: { $gt: 0 } }, { charmMonth: 0 });
  console.log('-lz- update stop ' + Date.now());
  ctx.logger.info('定时任务执行结果' + JSON.stringify(result));
}


module.exports = MonthSubscription;
